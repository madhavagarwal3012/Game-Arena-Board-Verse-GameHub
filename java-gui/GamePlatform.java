...
class TicTacToeGame extends JPanel {
    ...
    private List<int[]> winningCoords = new ArrayList<>();
    private boolean winningLineExists = false;
    ...
    private class ModernTicTacToeBoard extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            // Perfect, soothing grid0 lines (single, thin, very light gray)
            ...
            if (winningLineExists) {
                for (int[] coord : winningCoords) {
                    ...
                    // Semi-transparent green fill (rounded rectangle)
                    g2.setColor(new Color(0, 255, 0, 80));
                    g2.setStroke(new BasicStroke(Math.max(4, cellSize / 12), BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
                    g2.drawRoundRect(cx + 4, cy + 4, cellSize - 8, cellSize - 8, 24, 24);
                }
            }
            g2.dispose();
        }
    ...
    private boolean checkWinner(int row, int col) {
        ...
        if (count >= boardSize) {
            winningCoords.clear();
            for (int i = 0; i < boardSize; i++) {
                winningCoords.add(new int[]{row, i});
            }
            winningLineExists = true;
            return true;
        }
        ...
        if (count >= boardSize) {
            winningCoords.clear();
            for (int i = 0; i < boardSize; i++) {
                winningCoords.add(new int[]{i, col});
            }
            winningLineExists = true;
            return true;
        }
        ...
        if (count >= boardSize) {
            winningCoords.clear();
            for (int i = 0; i < boardSize; i++) {
                winningCoords.add(new int[]{i, boardSize - 1 - i});
            }
            winningLineExists = true;
            return true;
        }
...
class OthelloGame extends JPanel {
    ...
    private void initializeComponents() {
        ...
        startButton.addActionListener(e -> {
            ...
            initializeBoard();
            highlightCurrentPlayer(); // <-- ensure highlight after switching to game
        });
        ...
        // --- ADD THIS: highlight current player on UI setup ---
    ...
    private void highlightCurrentPlayer() {
        if (player1Panel != null && player2Panel != null) {
            ...
            // Highlight current player (Black = "B", White = "W")
            if ("B".equals(currentPlayer)) {
                player1Panel.setBackground(new Color(220, 255, 220)); // light green
                player1Panel.setBorder(BorderFactory.createCompoundBorder(
                    BorderFactory.createLineBorder(new Color(16, 185, 129), 5), // teal border
                    BorderFactory.createEmptyBorder(30, 26, 30, 26)
                ));
            } else if ("W".equals(currentPlayer)) {
                player2Panel.setBackground(new Color(220, 255, 220));
                player2Panel.setBorder(BorderFactory.createCompoundBorder(
                    BorderFactory.createLineBorder(new Color(16, 185, 129), 5),
                    BorderFactory.createEmptyBorder(30, 26, 30, 26)
                ));
            }
            player1Panel.repaint();
